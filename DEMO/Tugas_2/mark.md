...
for i in range(0,8):
        while 1:
          ...
          if self.board[rand_row][rand_col] == 0:
            ...
            break
```
Untuk menginisiasi queen pada tempat yang random
```py
def __repr__(self):
    mstr = ""
    for i in range(0,8):
    ...
    return (mstr)
```
Define untuk bagaimana print board
```py
def __init__(self, numruns, verbocity, passedboard=None):
    self.totalruns = numruns
    self.totalsucc = 0
...
def hill_solution(self):
    ...
    return self.cost
```
Implementasi hill climbing pada program
```py
def printstats(self):
    print ("Total Runs: ", self.totalruns)
    print ("Total Success: ", self.totalsucc)
...
def calc_cost(self, tboard):
    ...
    return ((totaldcost + totalhcost)/2)
```
Untuk mengecek queen apakah sudah sesuai atau belum
```py
  def getlowercostboard(self):
    lowcost = self.calc_cost(self.mboard)
    lowestavailable = self.mboard
    ...
    self.cost = lowcost
```
Fungsi ini mencoba memindahkan setiap queen ke setiap tempat, dengan hanya satu gerakan dan 
return yang memiliki jumlah pelanggaran paling sedikit
```py
if __name__ == "__main__":
  ...
  mboard.printstats()
```
Fungsi main untuk menjalankan program. Argument `-q` atau `--quite` agar tidak print step-step pengerjaan dan langsung print statistik. 
`--numrun` merupakan jumlah berapa kali di run (number of random board).
